...
**新功能必須同步新增對應斷言**（selftest 目前 104 項）。自測不可只靠截圖：必須含真實
...
- `tools/extract_mep.py` — DXF→mep_data.json（水電圖層：馬桶排水/排水立管/管道間；
  **建商此份 DWG 無插座/弱電/配電箱圖元**，已全層掃描確認；取得水電圖 CAD 後在此擴充 kind）
...
材質（玻璃/洞洞板/不鏽鋼）｜風格系統（32 種、styleSel、語意角色換色、覆蓋全部顏色含地板）｜
水電配置圖層（btnMep 唯讀切換、2D svg 層+3D 地面標記、CAD 精準座標）｜擬真外皮（addCyl 真圓柱 N=10：電鍋/垃圾桶/掛鍋；rr 圓角面；
