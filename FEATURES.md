...
- 單選紅色警示物件時，header 下方置中常駐顯示原因（不像 hint 淡出）：badReason(f)＝
  覆蓋牆面／與「某物」重疊（含高度區間）／超過天花板（含此物頂端高）。
  syncPanel 內更新；恢復合法或取消選取即隱藏。**測試擺位注意 fixtures（邊櫃 y882.5 起）與牆（x415-425）**。
...
- **addCyl 真圓柱**（rebuildBoxes 內、N=10 側板+頂底圓面，faces=12/柱）：SKIN3D 第 4 參數傳入，
  主 3D 用；cab Modal/櫃內 wrapper 無 cyl → 退回 rr 圓角箱。電鍋＝3 圓柱（鍋身可改色、
  頂蓋/蓋鈕固定不鏽鋼色）36 faces；垃圾桶＝2 圓柱；掛件鍋子 cyl。
- addBox 第 9 參數 rr（頂/底面 border-radius）：安全帽 46%、人頭 45%、鞋等沿用。
- **測試注意：8777 無快取標頭，改版後驗證需 cache-buster（?v=xxx）否則拿到舊檔**。
...
- 入口：面板「⤵ 垂直90°」btnRotV（未選取＝互換表單 fD/fH，作用於下次新增）、懸浮列「⤵ 立倒」btnBarRotV、
  V 鍵（無 Ctrl、選取中）。**人物／已編輯櫃體（hasCab）不支援**（櫃體內部座標依附 w/d/h 會毀損），
  多選時僅套用支援者並提示。
- **管柱特例（2026-08-01 使用者兩次修訂後定案）：翻轉 L/U 方向 f.pipeUp**——⌐（頂橫段＋垂直段朝下＝
  天花板轉折下牆）⇄ ⌟（底橫段＋垂直段朝上＝吊桿，短邊頂端釘天花板）。**外框尺寸不變**：長=橫段長、
  高=垂直段長、寬=管厚；bbox 不動所以釘頂（z+h=CEIL）自然保持。pipeUp 需隨 copy/dup/addFurniture 傳遞。

## 選取懸浮功能列

- #selBar（單選時顯示於物件上方）：🗄 櫃體／⟳90°／⤵ 立倒／⧉ 複製／🗑 刪除（人物無櫃體鈕）。
- **3D 顯示位置＝「整疊最高點」上方 84px**（掃 obbOverlap 之非人物物件取 max(z+h)；46px 仍會擋到本體，使用者回報後加大）：
...
- **共用尺寸/顏色欄（2026-08-01 三改）**：ciAddW/D/H＋ciAddColor 為唯一一組（舊 ciW/ciD/ciH/ciColor
  已移除）——**選取物品＝即時編輯該物品；未選＝新增前輸入值**。選放置格（點空白格）或改物品種類
  下拉＝自動取消物品選取進入新增模式；cabAddItem 後自動選取新物品（共用欄轉為編輯它）。
  ciName/ciY（深度位置）仍屬物品編輯區。尺寸輸入若 cabItemBad→**禁止並還原前值＋警示「超出放置空間」**
  （舊「顯紅允許」規則作廢，測試 50 已改寫）。
